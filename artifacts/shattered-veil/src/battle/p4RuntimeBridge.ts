...
function battleActionsCard() { return document.querySelector('.battle-bg .battle-actions-card') as HTMLElement | null; }
function battleControlRail() { return document.querySelector('.battle-bg .sv-battle-control-rail, .battle-bg .battle-info-card') as HTMLElement | null; }
function battleArenaPanel() { return document.querySelector('.battle-bg .sv-arena-panel') as HTMLElement | null; }
function battleTurnText() { return textOf(document.querySelector('.battle-bg .battle-turn-head:not(.sv-chronicle-turn-clone), .battle-bg .sv-turn-head-v101')); }
function isPlayerTurn() { const text = battleTurnText(); return /Your Turn/i.test(text) && !/Enemy Turn/i.test(text); }

function resetForNewTurnIfNeeded() {
  const turnText = battleTurnText();
  if (!document.querySelector('.battle-bg')) {
    p4MoveMode = null; p4RuntimeAp = 100; p4LastTurnText = ''; p4StrategicStepUsed = false; p4MoveTilesSpent = 0; return;
  }
...
function hideLegacyTacticalStep() { const wrap = legacyTacticalCardWrap(); if (wrap) { wrap.dataset.svP4HiddenTacticalStep = '1'; wrap.style.display = 'none'; } }
...
function restoreAuxiliaryActionsPlacement() {
  ...
  aux.style.display = '';
  aux.style.order = '25';
  if (bar && aux.previousElementSibling !== bar) rail.insertBefore(aux, bar.nextSibling);
}
...
function updateActionEconomyMirror() {
  ...
  if (!isPlayerTurn()) { state.textContent = 'Enemy turn'; fill.style.width = '0%'; return; }
  const nativeAp = Number.parseInt(bar.dataset.svNativeP4Ap || '', 10);
  if (Number.isFinite(nativeAp)) p4RuntimeAp = nativeAp;
...
function afterP4MoveCommitted(kind: 'basic_move' | 'strategic_step') {
  ...
  const arena = battleArenaPanel();
  if (arena) delete arena.dataset.svP4MoveKind;
  window.dispatchEvent(new CustomEvent('sv:p4-runtime-move-committed', { detail: { actionId: kind, remainingAp: p4RuntimeAp, moveTilesSpent: p4MoveTilesSpent, strategicStepUsed: p4StrategicStepUsed } }));
  updateActionEconomyMirror();
...
function ensureCombatMovementButtons() {
  ...
  if (step) { step.disabled = sourceDisabled || !isPlayerTurn() || p4RuntimeAp < P4_ACTION_COSTS.strategic_step.cost || p4StrategicStepUsed; step.title = P4_ACTION_COSTS.strategic_step.note; }
}
...
function tileDistanceFromPlayer(tile: Element) {
  ...
  const tx = tileIndex % firstRowTiles, ty = Math.floor(tileIndex / firstRowTiles), px = playerIndex % firstRowTiles, py = Math.floor(playerIndex / firstRowTiles);
...
function gateP4MovementClick(ev: Event) {
  ...
  if (p4MoveMode === 'basic_move' && dist !== 1) { ev.preventDefault(); ev.stopPropagation(); if (typeof (ev as Event & { stopImmediatePropagation?: () => void }).stopImmediatePropagation === 'function') (ev as Event & { stopImmediatePropagation: () => void }).stopImmediatePropagation(); return; }
  if (p4MoveMode === 'strategic_step' && (dist == null || dist < 1 || dist > 5)) { ev.preventDefault(); ev.stopPropagation(); if (typeof (ev as Event & { stopImmediatePropagation?: () => void }).stopImmediatePropagation === 'function') (ev as Event & { stopImmediatePropagation: () => void }).stopImmediatePropagation(); return; }
...
export function ensureP4RuntimeControls() {
  ...
  installRuntimeStyles(); polishLeftCommandTabs(); restoreAuxiliaryActionsPlacement(); ensureCombatMovementButtons(); updateActionEconomyMirror();
}
...
export function startP4RuntimeBridge() {
  ...
  p4RuntimeObserver.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ['class', 'disabled', 'style', 'data-sv-strategic-view'] });
