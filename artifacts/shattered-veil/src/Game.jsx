...
function Game() {
  ...
  // v92b — Mobile long-press tile inspector. Touch a world tile and hold
  // ~520ms to see compact tile info (terrain, coords, POI, dev/loot/etc).
  // Lifting early or sliding the finger >12px aborts the press so it never
  // interferes with tap-to-walk.
  const tileLongPressTimerRef = useRef(null);
  const tileLongPressStartRef = useRef(null);
  const tileLongPressFiredRef = useRef(false);
...
const buildGroupedBattleLog = (entries) => {
  ...
  // ── VEILCOURT (global chat) — v65 rewrite ──
  ...
  const veilcourtFetch = useCallback(async () => {
    try {
      const r = await fetch("/api/veilcourt/messages?since=" + chatLastIdRef.current + "&pid=" + encodeURIComponent(veilcourtId), { headers: { "Cache-Control": "no-cache" } });
      if (!r.ok) return;
      const data = await r.json().catch(() => null);
      if (!data) return;
      if (Array.isArray(data.messages) && data.messages.length > 0) {
        const incomingMaxId = data.messages.reduce((mx, m) => m.id > mx ? m.id : mx, 0);
        if (incomingMaxId > chatLastIdRef.current) chatLastIdRef.current = incomingMaxId;
        setChatMsgs(prev => {
          if (prev.length === 0) return data.messages.slice(-100);
  ...
  const renderRich = (text) => {
    ...
    return parts.map((p, i) => {
      ...
      const yt = /(?:youtube\.com\/watch\?v=|youtu\.be\/|youtube\.com\/shorts\/|youtube\.com\/embed\/)([\w-]{6,})/i.exec(cleanUrl);
      if (isImg) {
        return <span key={i}><a href={cleanUrl} target="_blank" rel="noopener noreferrer"><img className="veil-media veil-media-img" src={cleanUrl} alt="shared image" loading="lazy" referrerPolicy="no-referrer" onError={e => { try { const t = e.currentTarget; t.style.display = "none"; const fb = t.parentElement?.nextElementSibling; if (fb) fb.style.display = "inline"; } catch (_) {} }} /></a><a className="veil-link veil-link-fb" href={cleanUrl} target="_blank" rel="noopener noreferrer" style={{ display: "none" }}>{cleanUrl}</a>{trail}</span>;
      }
      if (yt) {
        const ytId = yt[1].slice(0, 11);
        return <span key={i}><span className="veil-media veil-media-yt"><iframe src={"https://www.youtube-nocookie.com/embed/" + ytId} title="YouTube video" loading="lazy" sandbox="allow-scripts allow-same-origin allow-presentation allow-popups" allow="encrypted-media; picture-in-picture" allowFullScreen frameBorder="0" referrerPolicy="strict-origin-when-cross-origin" /></span>{trail}</span>;
      }
      return <span key={i}><a className="veil-link" href={cleanUrl} target="_blank" rel="noopener noreferrer">{cleanUrl}</a>{trail}</span>;
    });
  };
  ...
  // Profile dossier modal — opens when a portrait is clicked
  const chatProfileEl = chatProfile ? (() => {
    const m = chatProfile;
    const isSystem = m.channel === "system";
    const portraitSrc = m.portrait && /^(https?:|data:image\/)/i.test(m.portrait) ? m.portrait : (m.classId ? ((import.meta.env.BASE_URL || "/") + "class/" + m.classId + (m.sex === "female" ? "_f" : "") + ".png") : null);
    return (
      <div>
        <div style={{ position: "fixed", inset: 0, zIndex: 1200, background: "rgba(2,4,12,0.85)", backdropFilter: "blur(4px)" }} onClick={() => setChatProfile(null)} />
        <div className="veil-profile-modal" onClick={e => e.stopPropagation()}>
          <button className="bt bs veilcourt-close" style={{ position: "absolute", top: 10, right: 10, zIndex: 2 }} onClick={() => setChatProfile(null)} aria-label="Close">✕</button>
          <div className="veil-profile-portrait" style={{ borderColor: m.classColor || (isSystem ? "#d4ad40" : "#7a89c2") }}>
            {portraitSrc ? <img src={portraitSrc} alt={m.name} onError={e => { try { e.currentTarget.style.display = "none"; } catch (_) {} }} /> : <span className="veil-profile-portrait-fb">{isSystem ? "🜂" : "✦"}</span>}
          </div>
          <div className="veil-profile-info">
            <div className="veil-profile-name" style={{ color: m.classColor || "#f5e8b8" }}>{m.name}</div>
            <div className="veil-profile-tagline">{m.className || (isSystem ? "Veilcourt herald" : "Sorcerer of the rift")}</div>
            <div className="veil-profile-grid">
              {m.className && (<div className="veil-profile-row"><span className="veil-profile-label">Class</span><span className="veil-profile-val" style={{ color: m.classColor || "#fff" }}>{m.className}</span></div>)}
              {m.rank &&      (<div className="veil-profile-row"><span className="veil-profile-label">Rank</span><span className="veil-profile-val">{m.rank}</span></div>)}
              {m.covenant &&  (<div className="veil-profile-row"><span className="veil-profile-label">Covenant</span><span className="veil-profile-val" style={{ color: "#ffd77a" }}>🏛 {m.covenant}</span></div>)}
              {m.bloodmark && (<div className="veil-profile-row"><span className="veil-profile-label">Bloodmark</span><span className="veil-profile-val" style={{ color: "#c2a8ff" }}>✦ {m.bloodmark}</span></div>)}
        ...
        <div className="cd page-panel" style={{ padding: 10 }}>
          ...
          <div ref={swipeRef} className="battle-world-grid" style={{ display: "grid", gridTemplateColumns: "repeat(" + VW + ",1fr)", gap: 1, marginBottom: 0, width: "100%", background: T.bg, borderRadius: 6, overflow: "hidden", border: "1px solid " + T.bd, animation: "mapMove .15s ease" }} key={pos.x + "," + pos.y}>
            {Array.from({ length: VW * VH }).map(function(_, idx) {
              ...
              return <div key={idx} className={isMe ? "world-tile is-player" : "world-tile"} data-cls={isMe ? (pl?.cid || cls?.id || "default") : undefined} title={isDev ? "Roaming Boss — click to walk · double-click to engage" : (hasPoi ? ((t.poi.nm || t.poi.type) + " — click to walk · double-click to enter") : "Click to walk here")} onClick={function() { if (tileLongPressFiredRef.current) { tileLongPressFiredRef.current = false; return; } if (gx === pos.x && gy === pos.y) { if (hasPoi) { autoEnterRef.current = false; enterPoi(); } return; } autoEnterRef.current = false; if (isDev) setTip(roamingBossIcon + " Roaming Boss at (" + gx + "," + gy + ")"); else if (hasPoi) setTip(t.poi.ic + " " + (t.poi.nm || t.poi.type) + " (" + t.poi.type + ") at (" + gx + "," + gy + ")"); if (gx >= 0 && gx < MW && gy >= 0 && gy < MH) setAutoMoveTarget({ x: gx, y: gy }); }} onDoubleClick={function(ev){ ev.preventDefault(); if (gx === pos.x && gy === pos.y) { if (hasPoi) enterPoi(); return; } if (gx < 0 || gx >= MW || gy < 0 || gy >= MH) return; autoEnterRef.current = !!hasPoi; setAutoMoveTarget({ x: gx, y: gy }); }} onContextMenu={function(ev){ if (gx < 0 || gx >= MW || gy < 0 || gy >= MH) return; ev.preventDefault(); var lines = ["📍 (" + gx + ", " + gy + ")"]; if (isMe) lines.push("👤 You are here."); if (isDev) lines.push("⚠ Roaming Boss — double-click to engage."); if (t && t.bio) lines.push("🌍 Terrain: " + (t.bio.charAt(0).toUpperCase() + t.bio.slice(1))); if (hasPoi) { lines.push((t.poi.ic || "✦") + " " + (t.poi.nm || t.poi.type) + " (" + t.poi.type + ")"); if (t.poi.ds) lines.push(t.poi.ds); } else if (!isDev) { lines.push("— No landmark on this tile —"); } setPopup({ text: lines.join("\n"), choices: [{ label: "Close", action: function(){ setPopup(null); } }] }); }} onTouchStart={function(ev){ if (gx < 0 || gx >= MW || gy < 0 || gy >= MH) return; var tch = ev.touches && ev.touches[0]; if (!tch) return; tileLongPressFiredRef.current = false; tileLongPressStartRef.current = { x: tch.clientX, y: tch.clientY }; if (tileLongPressTimerRef.current) clearTimeout(tileLongPressTimerRef.current); tileLongPressTimerRef.current = setTimeout(function(){ tileLongPressFiredRef.current = true; var lines = ["📍 (" + gx + ", " + gy + ")"]; if (isMe) lines.push("👤 You are here."); if (isDev) lines.push("⚠ Roaming Boss — double-tap to engage."); if (t && t.bio) lines.push("🌍 Terrain: " + (t.bio.charAt(0).toUpperCase() + t.bio.slice(1))); if (hasPoi) { lines.push((t.poi.ic || "✦") + " " + (t.poi.nm || t.poi.type) + " (" + t.poi.type + ")"); if (t.poi.ds) lines.push(t.poi.ds); } else if (!isDev) { lines.push("— No landmark on this tile —"); } setPopup({ text: lines.join("\n"), choices: [{ label: "Close", action: function(){ setPopup(null); } }] }); try { if (navigator && navigator.vibrate) navigator.vibrate(15); } catch(_){} }, 520); }} onTouchMove={function(ev){ var s = tileLongPressStartRef.current; var tch = ev.touches && ev.touches[0]; if (!s || !tch) return; var dx = tch.clientX - s.x, dy = tch.clientY - s.y; if (dx*dx + dy*dy > 144) { if (tileLongPressTimerRef.current) { clearTimeout(tileLongPressTimerRef.current); tileLongPressTimerRef.current = null; } } }} onTouchEnd={function(){ if (tileLongPressTimerRef.current) { clearTimeout(tileLongPressTimerRef.current); tileLongPressTimerRef.current = null; } tileLongPressStartRef.current = null; }} onTouchCancel={function(){ if (tileLongPressTimerRef.current) { clearTimeout(tileLongPressTimerRef.current); tileLongPressTimerRef.current = null; } tileLongPressStartRef.current = null; }} style={{ position: isMe ? "relative" : undefined, overflow: isMe ? "hidden" : undefined, aspectRatio: "1", background: bgC, color: isTrail ? "#f2c45c" : isDev ? "#ffd0d0" : hasPoi ? (poiBorder || "#fff") : (t && t.bio === "ocean" ? "#bfe6ff" : "#edf4ff"), display: "flex", alignItems: "center", justifyContent: "center", fontSize: isMe ? 13 : isTrail ? 9 : hasPoi ? 9 : decor ? 8 : 6, borderRadius: 2, border: isMe ? "1.5px solid " + T.gd : poiBorder ? "1.5px solid " + poiBorder : "1px solid rgba(255,255,255,0.05)", boxShadow: isMe ? ((t && t.bio === "ocean") ? "0 0 16px rgba(0,180,255,0.45), inset 0 0 0 1px rgba(255,255,255,0.18)" : "0 0 14px rgba(242,196,92,0.45), inset 0 0 0 1px rgba(255,255,255,0.12)") : poiBorder ? poiRing(hasPoi ? t.poi.type : "dev") : "inset 0 0 0 1px rgba(255,255,255,0.04)", opacity: t && t.bio === "ocean" ? 0.95 : 1, cursor: "pointer", transition: "background .15s, border .15s, box-shadow .15s", textShadow: isTrail || hasPoi || isDev ? "0 0 8px rgba(0,0,0,0.45)" : "none" }}>{cellLabel}{meContent}{isMe ? <span className="player-aura-ring" aria-hidden="true" /> : null}</div>;
            })}
          </div>
          </div>
        </div>
