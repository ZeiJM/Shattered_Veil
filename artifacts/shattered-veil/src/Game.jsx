...
const buildGroupedBattleLog = (entries) => {
  ...
  const popupEl = popup ? <div><div style={{ position: "fixed", inset: 0, zIndex: 1001, background: "rgba(0,0,0,0.6)" }} onClick={() => { if (!popup.choices && !popupJustOpenedRef.current) setPopup(null); }} onTouchStart={(ev) => { if (!popup.choices && !popupJustOpenedRef.current) { ev.stopPropagation(); setPopup(null); } }} /><div style={popup.fullscreen ? { position: "fixed", top: "50%", left: "50%", transform: "translate(-50%,-50%)", background: "#1a1e38", border: "1px solid #3a3f62", borderRadius: 10, padding: 14, zIndex: 1002, maxWidth: 360, width: "88%", maxHeight: "72vh", boxShadow: "0 8px 32px rgba(0,0,0,0.7)", overflowY: "auto", overflowX: "hidden" } : { position: "fixed", top: "50%", left: "50%", transform: "translate(-50%,-50%)", background: "#1a1e38", border: "1px solid #3a3f62", borderRadius: 10, padding: 16, zIndex: 1002, maxWidth: 340, width: "90%", boxShadow: "0 8px 32px rgba(0,0,0,0.7)" }} onClick={e => e.stopPropagation()}>
    {popup.title ? <div style={{ fontSize: popup.fullscreen ? 14 : 12, color: T.gd, fontWeight: 800, textAlign: "center", marginBottom: 10 }}>{popup.title}</div> : null}
    <div style={{ fontSize: popup.fullscreen ? 13 : 12, color: T.tx, marginBottom: 10, lineHeight: 1.5, whiteSpace: "normal", display: "flex", justifyContent: "center", width: "100%" }}>{popup.node ? <div style={{ width: "100%", minWidth: 0 }}>{popup.node}</div> : renderPopupText(popup.text)}</div>
    {popup.choices ? <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>{popup.choices.map((c, i) => <button key={i} className="bt" style={{ background: T.c2, width: "100%", textAlign: "left" }} onClick={c.action}>{c.label}</button>)}</div> : <div style={{ display: "flex", justifyContent: "center" }}><button className="bt" style={{ background: T.c2, width: popup.fullscreen ? "auto" : "100%", minWidth: popup.fullscreen ? 120 : undefined }} onClick={() => setPopup(null)}>{popup.fullscreen ? "Close" : "OK"}</button></div>}
  </div></div> : null;
...
{sub === "items" && (() => {
  ...
  if (scr === "battle" && btl) {
    ...
    return (
      <div className="pg battle-bg"><div className="wr battle-viewport" style={{position:"relative",zIndex:1}}>{notiEl}{tipEl}{popupEl}
        ...
        <div className="battle-top-grid">
          <div className="battle-team-col">
            <div className="cd" style={{ padding: 6, marginBottom: 3 }}>
              ...
              <div style={{ padding: 4, background: T.c2, borderRadius: 4, fontSize: 9, display: "flex", gap: 6, alignItems: "center" }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 9, color: T.dm }}>HP:{pl.chp}/{effSt(pl).hp} · MP:{pl.cmp}/{effSt(pl).mp} · SPD:<span style={{ color: spdColor(effSt(pl).spd), fontWeight: 700 }}>{effSt(pl).spd}</span></div>
                  <button type="button" className="bt bs" style={{ background: T.c2, color: "#9fd6ff", marginTop: 2, padding: "2px 6px", fontSize: 8 }} onClick={() => openElementSummaryPopup(entityBattleElements(pl), "player", battlePlayerName)}>Element Summary</button>
            ...
            {(ally || pet) && <div className="cd" style={{ padding: 6, marginBottom: 3 }}>
              ...
              {pet && (pet.chp ?? pet.hp ?? 0) > 0 && <div style={{ padding: 4, background: T.c2, borderRadius: 4, marginBottom: 2, fontSize: 9, display: "flex", gap: 6, alignItems: "center" }}><div style={{ width: 28, height: 28, borderRadius: 6, background: (ELC[(entityBattleElements(pet)[0]) || pet.el]||T.ac)+"22", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>{pet.ic}</div><div style={{ flex: 1 }}><div style={{ fontWeight: 700, display: "flex", flexWrap: "wrap", gap: 4, alignItems: "center", fontSize: 11 }}><span>{pet.nm}</span>{entityBattleElements(pet).map((elx, i) => <ElementTag key={elx + "_" + i} el={elx} fontSize={10} />)}</div>{pBar(pet.chp ?? pet.hp, pet.mhp ?? pet.hp, T.hp)}{pBar(pet.mp ?? pet.mmp ?? 0, pet.mmp ?? pet.mp ?? 1, T.mp)}<div style={{ fontSize: 9, color: T.dm }}>HP:{pet.chp ?? pet.hp}/{pet.mhp ?? pet.hp} · MP:{pet.mp ?? pet.mmp ?? 0}/{pet.mmp ?? pet.mp ?? 0} · SPD:<span style={{ color: spdColor(pet.spd || 0), fontWeight: 700 }}>{pet.spd || 0}</span> · {pet.sk1?.n || "?"} / {pet.sk2?.n || "?"}</div><button type="button" className="bt bs" style={{ background: T.c2, color: "#9fd6ff", marginTop: 2, padding: "2px 6px", fontSize: 8 }} onClick={() => openElementSummaryPopup(entityBattleElements(pet), "ally", pet.nm)}>Element Summary</button><div className="battle-passive-line"><span style={{ color: "#bfc9ef" }}>Passive</span><button type="button" className="battle-passive-chip player" style={{ cursor: "pointer" }} onClick={() => setPopup({ text: passivePopupText(pet.passive || pet.passiveName || "None", pet.passive || pet.passiveName ? ("Pet instinct: " + (pet.passiveBonus || "damage") + "-leaning support for your battle rhythm.") : null) })}>{pet.passive || pet.passiveName || "None"}</button></div></div></div>}
              {ally && ally.hp > 0 && <div style={{ padding: 4, background: T.c2, borderRadius: 4, fontSize: 9, display: "flex", gap: 6, alignItems: "center" }}><div style={{ width: 28, height: 28, borderRadius: 6, background: (ELC[(entityBattleElements(ally)[0]) || ally.el]||T.ok)+"22", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>{ally.ic||"🤝"}</div><div style={{ flex: 1 }}><div style={{ fontWeight: 700, display: "flex", flexWrap: "wrap", gap: 4, alignItems: "center", fontSize: 11 }}><span>{ally.nm}</span>{entityBattleElements(ally).map((elx, i) => <ElementTag key={elx + "_" + i} el={elx} fontSize={10} />)}</div>{pBar(ally.hp, ally.mhp, T.hp)}{pBar(ally.mp ?? ally.mmp ?? ally.cls?.st?.mp ?? 0, ally.mmp ?? ally.cls?.st?.mp ?? 1, T.mp)}<div style={{ fontSize: 9, color: T.dm }}>HP:{ally.hp}/{ally.mhp} · MP:{ally.mp ?? ally.mmp ?? ally.cls?.st?.mp ?? 0}/{ally.mmp ?? ally.cls?.st?.mp ?? 0} · SPD:<span style={{ color: spdColor(ally.spd || 0), fontWeight: 700 }}>{ally.spd || 0}</span> · {ally.sk1?.n || "?"} / {ally.sk2?.n || "?"}</div><button type="button" className="bt bs" style={{ background: T.c2, color: "#9fd6ff", marginTop: 2, padding: "2px 6px", fontSize: 8 }} onClick={() => openElementSummaryPopup(entityBattleElements(ally), "ally", ally.nm)}>Element Summary</button><div className="battle-passive-line"><span style={{ color: "#bfc9ef" }}>Passive</span><button type="button" className="battle-passive-chip player" style={{ cursor: "pointer" }} onClick={() => setPopup({ text: passivePopupText(ally.passive || ally.passiveName || "None", ally.passive || ally.passiveName ? ("Ally role: " + (ally.role || ally.passiveBonus || "support") + ". " + (ally.passiveName || ally.passive || "Shrine Blessing") + " shapes how this ally contributes.") : null) })}>{ally.passive || ally.passiveName || "None"}</button></div></div></div>}
            </div>}
          </div>
          <div className="battle-enemy-col">
            <div className="cd" style={{ padding: 6, marginBottom: 3 }}>
              <div style={{ fontSize: 9, fontWeight: 700, color: T.bad, marginBottom: 3 }}>Enemies</div>
              {[...btl.en].sort((a, b) => { const av = a.hp > 0 ? 1 : 0; const bv = b.hp > 0 ? 1 : 0; return bv - av; }).map(e => <div key={e.id} onClick={() => e.hp > 0 && setBtlTarget(e.id)} style={{ padding: 4, background: btlTarget === e.id ? T.bad + "18" : T.c2, borderRadius: 5, marginBottom: 2, opacity: e.hp <= 0 ? 0.2 : 1, cursor: e.hp > 0 ? "pointer" : "default", border: btlTarget === e.id ? "1px solid " + T.bad + "55" : "1px solid transparent", transition: "all .15s", display: "flex", gap: 6, alignItems: "center", overflow: "hidden" }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <button type="button" className="bt bs" style={{ background: T.c2, color: "#ffcfda", marginTop: 2, padding: "2px 6px", fontSize: 8 }} onClick={(ev) => { ev.stopPropagation(); openElementSummaryPopup(entityBattleElements(e), "enemy", e.name); }}>Element Summary</button>
