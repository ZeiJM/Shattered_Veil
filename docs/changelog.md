...
## v95 — Mobile UX Repair Pass (CSS-only, no JSX/engine changes)

Surgical CSS-only pass triggered by user mobile screenshots. Three concrete bugs fixed plus a foreshadow color token system for future targeting wiring. Save shape, combat math, world-gen, Veilbreak, Field Clash, marriage/heirs, chat all untouched.

- **Action card word-break** (`game.css` ~L1190) — added `word-break: keep-all`, `overflow-wrap: normal`, `hyphens: none` (+ `-webkit-hyphens: none`) to `.battle-action-card-wrap .battle-action-btn` and its `> div` children. Fixes the "Thornwo od Longbow" mid-word hyphenation seen in the user's screenshot. Paired with `clamp(10px, 2.8vw, 13px)` font sizing + `-webkit-line-clamp: 2` so long titles shrink/truncate cleanly instead of overflowing or wrapping mid-word.
- **Command dock labels restored on mobile** (`game.css` ~L3349) — the `@media (max-width: 460px)` rule used to set `.battle-bg .battle-tab-nm { display: none }` which left an icon-only dock on phones (visible in screenshot 4). Now `display: inline-block !important; font-size: 8px !important` so Combat / Veil / Tactical / Items / Veilbreak labels stay visible. Doubled-up the rule for `.battle-tab.v66` to win against the v66 chrome.
- **Arena painted-background fade** (`game.css` v95 block) — `.sv-arena-grid-sq.sv-arena-grid-has-bg::before` opacity dropped 0.42 → 0.18 with a tiny blur + saturate desat. Stops the brick-perspective `battle-arena.png` from fighting the v94 square grid lines.
- **World rail mobile compaction** — at `≤540px` the `.rail-action-btn` (giant ENTER/Idle/Stop), `.map-rail-quick` (HP/MP/Fish — was a vertical stack of three tall rectangles, now a 3-across row), `.map-rail-tile`, `.map-rail-compass`, `.map-rail-events`, `.map-rail-meta`, `.veil-quick-open-btn`, and Legend/Controls all shrink to compact paddings (4–6px) and smaller min-heights (28–38px). At `≤380px` the quick-button text labels drop (icon only). The world map itself is unchanged — it remains a fixed 15×9 viewport that re-renders centered on `pos.x,pos.y` (player at column 7, row 4 by design) — the rail simply takes less vertical room before the map appears.
